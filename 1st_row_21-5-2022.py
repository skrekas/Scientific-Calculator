...
class Calculator:
    ...
    def __init__(self):
        ...
        self.new_value= "" # k 21-5-222(ορίζει τη νέα τιμή στο backspace button
    ...
    def create_clear_entry_button(self): #k 21-5-2022
        button = tk.Button(self.buttons_frame, text="CE", bg=WHITE,
                           fg=DIGITS_COLOR, font=DIGITS_FONT, borderwidth=0,command=lambda: self.clear_entry())
        button.grid(row=0, column=2, columnspan=1, sticky=tk.NSEW)

    def precent(self): # k 21-5-2022
        curr=eval(self.current_value)
        self.current_value=str(curr*0.01)
        self.update_current_value()
        self.update_total_value()

    def create_precent_button(self):#k21-5-2022
        button = tk.Button(self.buttons_frame, text="%", bg=WHITE,
                           fg=DIGITS_COLOR, font=DIGITS_FONT, borderwidth=0,command=lambda: self.precent())
        button.grid(row=0, column=3, columnspan=1, sticky=tk.NSEW)

    def backspace(self):#k-21-5-2022
        print(font.families())
        self.current_value = self.current_value[:len(self.current_value) - 1]
    ...
    def create_backspace_button(self):#k-21-5-2022
        button = tk.Button(self.buttons_frame, text="\u232b", bg=WHITE,
                           fg=DIGITS_COLOR, font=DIGITS_FONT, borderwidth=0, command=lambda: self.backspace())
        button.grid(row=0, column=4, columnspan=1, sticky=tk.NSEW)
